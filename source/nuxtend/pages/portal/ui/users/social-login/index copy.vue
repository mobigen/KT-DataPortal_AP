...
export default {
  ...
  async asyncData({ params, query, req, res, error }) {
    ...
    // console.log("req.attributes : ", req.attributes);
    // console.log("res : ", res);
  ...
  beforeMount() {
    ...
    console.log("this.socialLoginType : ", this.socialLoginType);
    console.log("this.oAuthAccessToken : ", this.oAuthAccessToken);
  ...
  methods: {
    ...
    async checkSocialMember() {
      ...
      const data = await this.socialLogin(socialLoginType);
      if (data) {
    ...
    async socialLogin(socialLoginType) {
      const config = {
        params: {
